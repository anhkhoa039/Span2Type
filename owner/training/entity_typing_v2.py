...
def _triplet_valid_anchor_fraction(entity_types: torch.Tensor) -> float:
    """Share of batch anchors that have at least one same-type and one different-type
    other entity (required for triplet loss). Low values explain flat metrics: many
    steps contribute little or no gradient for rare types in random batches.
    """
    ...
    for i in range(batch_size):
        ...
        if same_type.any() and diff_type.any():
            valid += 1
...
class EntityTypingTrainer(BaseTrainer):
    ...
    def train_one_epoch(self, epoch: int, train_dataloader: DataLoader, test_dataloader: DataLoader):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), total=train_dataloader_len):
            ...
            mlflow.log_metrics({
                'et_train_batch_triplet-margin-loss': float(loss.detach()),
                'et_train_triplet_valid_anchor_frac': (
                    _triplet_valid_anchor_fraction(entity_type_labels)),
            }, step=current_step)
        self.model.eval()
