...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=north west,inner sep=0pt,xshift=0.9cm,yshift=-0.45cm]
      at (current page.north west) {\includegraphics[height=0.95cm]{cgu logo2.png}};
    \node[anchor=north east,inner sep=0pt,xshift=-0.9cm,yshift=-0.45cm]
      at (current page.north east) {\includegraphics[height=0.95cm]{aidept logo.png}};
...
\begin{frame}{Two Naming Backends: MLM and LLM}
  ...
  \begin{center}
    ...
    \begin{tabular}{lccc}
      ...
      \textbf{LLM (MMR)} & \cmark & \cmark & \textbf{0.924} \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{tabular}{ll}
        ...
        \midrule
        Detector backbone & BART-based \\
        Detector LR       & $1\times10^{-5}$, 50 epochs \\
        EMA decay         & $\alpha = 0.995$ \\
        \midrule
        Typing backbone   & DeBERTav3 \\
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{tabular}{l|ccccc|c}
        ...
        \#GT Types & 14 & 12 & 13 &  9 & 17 & 65 \\
        \midrule
        OWNER MLM  & .900 & .891 & .906 & .892 & .894 & .897 \\
        OWNER LLM (rand.) & .892 & .923 & .928 & .910 & .886 & .908 \\
        \midrule
        S2T Centroid & --- & --- & --- & --- & --- & .893 \\
        S2T MLM+MMR &.900 & .889 & .911 & .887 & .893 & .896 \\
        \rowcolor{gold!25}
        \textbf{S2T LLM+MMR} & \textbf{.922} & .913 & \textbf{.943} & \textbf{.928} & \textbf{.912} & \textbf{.924} \\
        \bottomrule
      \end{tabular}
      \vspace{0.4em}
      \normalsize
      ...
      \begin{itemize}
        \item Span2Type-LLM (MMR): \textbf{0.924}
        \item OWNER (LLM random): 0.908
        \item \hg{MMR gain: $+0.016$ avg}
        \item Lit exception: OWNER LLM (0.923) $>$ ours (0.913)
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}{Discussion: Key Findings \& Insights}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item Literature: OWNER LLM random (0.923) $>$ S2T MMR (0.913)
        \item Literary clusters are \textbf{tight \& homogeneous}:\\
          \textit{writer}, \textit{novel}, \textit{literarygenre} — narrow semantic field
    ...
    \begin{column}{0.6\textwidth}
      ...
      \begin{enumerate}
        ...
        \item \textbf{MMR-based cluster naming}\\
          ...
          LLM backend: \textbf{0.924} avg BERTScore-F1\\
          \hg{$+0.016$ over random selection} (budget-controlled)
      \end{enumerate}
    \end{column}
    \begin{column}{0.38\textwidth}
      \begin{tikzpicture}
...
% ------------------------------------------------------------------
% APPENDIX E — AMI EXPLAINED
% ------------------------------------------------------------------
\begin{frame}{Appendix E — AMI: Adjusted Mutual Information}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \textbf{What it measures}\\[2pt]
      \small
      Given gold types $U$ and predicted clusters $V$, AMI asks:\\[2pt]
      \emph{"How much does a span's cluster reveal its true type, \textbf{beyond chance}?"}\\[5pt]

      \textbf{Formula}\\[2pt]
      \footnotesize
      \[
        \mathrm{AMI}(U,V) =
          \frac{MI(U,V) - \mathbb{E}[MI]}
               {\tfrac{1}{2}[H(U)+H(V)] - \mathbb{E}[MI]}
      \]
      \small
      \begin{itemize}
        \setlength{\itemsep}{2pt}
        \item $MI$ — shared information between the two clusterings
        \item $\mathbb{E}[MI]$ — expected MI under random permutation
        \item $H$ — entropy (normaliser)
      \end{itemize}
      \vspace{4pt}
      \textbf{Why not F1?}\\[2pt]
      Open-world NER outputs \textit{Cluster 3}, not \textit{"ORG"}. AMI evaluates \textbf{partition structure} without requiring label alignment.
    \end{column}
    \begin{column}{0.44\textwidth}
      \textbf{Toy example}\\[4pt]
      \footnotesize
      \begin{tabular}{lll}
        \toprule
        \textbf{Span} & \textbf{Gold} & \textbf{Cluster} \\
        \midrule
        OpenAI  & ORG  & C2 \\
        Google  & ORG  & C2 \\
        DeepMind& ORG  & C2 \\
      ...
      C2 $\leftrightarrow$ ORG, C5 $\leftrightarrow$ PER, C8 $\leftrightarrow$ MISC\\
      \hg{$\Rightarrow$ AMI $\approx$ 1.0 (perfect)}\\[10pt]

      If all spans $\to$ one cluster:\\
      \hg{$\Rightarrow$ AMI $\approx$ 0 (random)}\\[10pt]

      \textbf{Span2Type results}\\[3pt]
      \begin{tabular}{lr}
        \toprule
        System & Avg AMI \\
        \midrule
